...
**Важное слово от разработчика!** *В репозитории с контейнеризацией я навыкаблучивался с Nginx’ом и словил какой-то баг - CDR по какой-то причине не может отправить запрос на CRM или Nginx. В, так скажем, dev-версии проекта этот баг устранён (хотя код там идентичный). Однако обращения через Swagger или Insomnia проходят в контейнерной версии*
